...
echo -e "Installing FoldCraft environment\n"
$pkg_manager create --name FoldCraft python=3.10 -y || { echo -e "Error: Failed to create FoldCraft conda environment"; exit 1; }
conda env list | grep -w 'FoldCraft' >/dev/null 2>&1 || { echo -e "Error: Conda environment 'FoldCraft' does not exist after creation."; exit 1; }
...
if [ -n "$cuda" ]; then
    CONDA_OVERRIDE_CUDA="$cuda" $pkg_manager install pip pandas matplotlib "numpy<2.0.0" biopython scipy pdbfixer seaborn libgfortran5 tqdm jupyter ffmpeg pyrosetta fsspec py3dmol chex dm-haiku "flax<0.10.0" dm-tree joblib ml-collections immutabledict optax cuda-nvcc cudnn -c conda-forge -c nvidia --channel https://conda.graylab.jhu.edu -y \
    && pip install "jax[cuda12_pip]==0.4.30" -f https://storage.googleapis.com/jax-releases/jax_cuda_releases.html \
    || { echo -e "Error: Failed to install packages."; exit 1; }
else
...
# finish
conda deactivate
echo -e "FoldCraft environment set up\n"
...
################## finish script
t=$SECONDS 
echo -e "Successfully finished FoldCraft installation!\n"
echo -e "Activate environment using command: \"$pkg_manager activate FoldCraft\""
echo -e "\n"
